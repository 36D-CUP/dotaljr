...
Class Index extends Allow
{
	...
	public function getAdminList()
	{
		...
		switch($s_type){
			case 1:
				$num  = Db::table('my_admin')->where($s_ty,'like',"%".$s_val."%")->count();							//数量
				$data = Db::table('my_admin')->where($s_ty,'like',"%".$s_val."%")->limit($str)->select();			//获取所有管理员
			break;
			default:
				$num  = Db::table('my_admin')->count();												//数量
				$data = Db::table('my_admin')->limit($str)->select();								//获取所有管理员
			break;
		}

		//组合用户图片
		$img  = Db::table('my_img')->where('img_surface_name','my_admin')->select();		//获取所有管理员图片
