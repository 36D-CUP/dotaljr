	...
	public function getNotice()
	{
		...
		switch($s_type){
			case 1:
				...
				$data = Db::table($table)->where($s_ty,'like',"%".$s_val."%")->limit($str)->select();			//获取所有
			break;
			default:
				$num  = Db::table($table)->count();												//数量
				$data = Db::table($table)->limit($str)->select();								//获取所有
			break;
		}

		//组合用户图片
		$img  = Db::table('my_img')->where('img_surface_name',$table)->select();		//获取所有图片
